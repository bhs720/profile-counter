...
These patches are deliberately not submitted upstream. Currently: `0001-test-device-colour-sampling.patch` fixes two defects in mupdf's test device (the device `FileAnalyzer` relies on for colour detection) that made it sample uninitialised memory, so the same file could be classified black-and-white or colour at random between runs — most importantly, both image-sampling call sites passed the `fz_color_converter` arguments in `(dst, src)` order when the declared order is `(src, dst)`.
