...
namespace ProFileCounter
{
	public partial class PageSizeManager : Form
	{
		...
		{
			InitializeComponent();
			// Store the page list in a DataGridView
			// When the user clicks OK, we'll turn it back into a List<PageSize>
		...
		void BtnEditClick(object sender, System.EventArgs e)
		{
			...
			using (var pse = new PageSizeEditor((editingRow.Tag as PageSize).Clone()))
			{
				pse.ShowDialog(this);
				if (pse.DialogResult == DialogResult.OK)
				{
